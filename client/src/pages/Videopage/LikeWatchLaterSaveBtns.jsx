...
function LikeWatchLaterSaveBtns({ vmd, vid }) {
  ...
  useEffect(() => {
    if (Array.isArray(likedvideoList?.data)) {
      likedvideoList?.data
        ?.filter((q) => q.vid === vid && q.viewer === currentUser?.result?._id)
        .map((m) => setLikeVideo(true));
    }
  ...
  const toggleSaveVideo = () => {
    if (currentUser) {
      if (saveVideo) {
        setSaveVideo(false);
        dispatch(
          deleteWatchLater({ vid: vid, viewer: currentUser?.result._id })
        );
      } else {
        setSaveVideo(true);
  ...
  const toggleLike = (e, lk) => {
    if (currentUser) {
      if (likeVideo) {
        ...
        dispatch(
          deleteLikedVideo({ vid: vid, viewer: currentUser?.result._id })
        );
      } else {
        setLikeVideo(true);
    ...
    <div className="btns_cont_videoPage">
      <div className="like_container">
        ...
        <div
          ...
          onClick={(e) => toggleDislike(e, vmd.like)}
        >
          {dislikeVideo ? (
